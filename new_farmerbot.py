...
class DataManager:
    ...
    def get_crop_data(self, crop: str, metric: str, years: List[int]) -> pd.DataFrame:
        ...
        try:
            cache_key = self.cache_manager.get_cache_key('crop_data', crop, metric, str(years))
            ...
            raw_result = list(self.collection.find(query))
            print(f"Raw result from database: {raw_result}")
            ...
            # Check for variations of 'Value' column name
            possible_value_columns = ['Value', 'value', 'VALUES', 'values', 'val', 'VAL']
            found_value_column = None
            for col in possible_value_columns:
                if col in df.columns:
                    found_value_column = col
                    break

            if not found_value_column:
                # If no standard value column found, try to identify it from available columns
                print(f"Available columns: {df.columns.tolist()}")
                numeric_columns = df.select_dtypes(include=['float64', 'int64']).columns
                print(f"Numeric columns: {numeric_columns.tolist()}")
                
                # Exclude Year from numeric columns
                numeric_columns = [col for col in numeric_columns if col != 'Year']
                
                if len(numeric_columns) == 1:
                    found_value_column = numeric_columns[0]
                elif len(numeric_columns) > 1:
                    # If multiple numeric columns, try to identify the most likely value column
                    for col in numeric_columns:
                        if any(val_term in col.lower() for val_term in ['val', 'amount', 'quantity', 'measure']):
                            found_value_column = col
                            break
                    if not found_value_column:
                        found_value_column = numeric_columns[0]  # Take the first numeric column as a last resort

            if not found_value_column:
                raise ValueError(f"Could not identify value column. Available columns: {df.columns.tolist()}")

            # Rename the found column to 'Value'
            df = df.rename(columns={found_value_column: 'Value'})
            ...
            # Select only needed columns, handling potential missing columns
            needed_columns = ['Year', 'Value', 'Item', 'Element']
            available_columns = [col for col in needed_columns if col in df.columns]
            df = df[available_columns]

            print(f"Final DataFrame head: {df.head()}")
    ...
    def get_data_summary(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            return {
                ...
                "Latest Year Value": round(df.loc[df['Year'].idxmax(), 'Value'], 2),
...
class ChatbotUI:
    ...
    def show_data_view(self) -> None:
        ...
        years = list(range(2015, 2024))  # Adjust year range as needed
        ...
        try:
            ...
            try:
                ...
                for (stat, value), col in zip(stats.items(), cols):
                    col.metric(label=stat, value=value)
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"{context['crop']}_{context['metric']}_data.csv",
                mime='text/csv'
                )
...
def main():
    ...
    try:
        ...
        # Handle current stage based on session state
        ...
        if st.button("Restart Application"):
            # Do NOT call st.session_state.clear() here; just reset necessary parts
